...
if userResponse.lower() == 'one':
    ...
    if int(deviceID) <= int(index - 1):  # Checks to see if number is within range
        ...
        with open(filename, "a") as file:
            ...
            for i in req_json:
                if i['name'] == sensorArray[newIndex] and i[
                    'connected'] == 1:  # Finds the sensor that was specified and extracts its data
                    deviceConnected = True
                    print "Device " + str(i['name']) + " is online"
                    # Access online devices API
                    onDevice = requests.get(
                        'https://api.particle.io/v1/devices/' + i['id'] + '?access_token=' + access_token)
                    ...
                    if 'variables' in onDevice_json:
                        for var in onDevice_json['variables']:
                            data = requests.get('https://api.particle.io/v1/devices/' + i[
                                'id'] + '/' + var + '?access_token=' + access_token)
                            onDeviceArray.append(str('https://api.particle.io/v1/devices/' + i[
                                'id'] + '/' + var + '?access_token=' + access_token))
                            data_json = data.json()
                        ...
                        if i['name'] == sensorArray[newIndex] and i['connected'] == 1 and i['name'] not in deviceName:
                            ...
                            if 'variables' in onDevice_json:
                                for var in onDevice_json['variables']:
                                    ...
                                    data_json = data.json()
                                    nameIndex.append(
                                        var + '_' + i['name'])  # add var name to index along with the device name
                                    if 'result' in data_json:
                                        print "Variable: " + str(var) + ": " + str(data_json['result'])
